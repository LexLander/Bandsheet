      ...
      <table className="w-full text-sm">
        <thead className="bg-black/5 dark:bg-white/5">
          <tr>
            ...
            <th className="text-left px-3 py-2">{t.admin.plans.tableName}</th>
            <th className="text-right px-3 py-2">{t.admin.plans.tablePrice}</th>
            <th className="text-right px-3 py-2">{t.admin.plans.tableYearlyPrice}</th>
            <th className="text-center px-3 py-2">{t.admin.plans.tableFree}</th>
            <th className="text-center px-3 py-2">{t.admin.plans.tableActive}</th>
            <th className="text-center px-3 py-2">{t.admin.plans.tableTrialDays}</th>
            <th className="text-left px-3 py-2">{t.admin.plans.tableActions}</th>
          </tr>
        </thead>
        <tbody>
          {plans.map((plan, idx) => (
            <tbody key={plan.id}>
              <tr className="border-t border-black/10 dark:border-white/10">
                <td className="px-3 py-2 text-xs text-foreground/60">{idx + 1}</td>
                ...
                <td className="px-3 py-2 text-center">{plan.is_free ? '✓' : '—'}</td>
                <td className="px-3 py-2 text-center">{plan.is_active ? '✓' : '—'}</td>
                <td className="px-3 py-2 text-center">{plan.trial_days}</td>
                <td className="px-3 py-2 space-x-2 flex flex-wrap gap-1">
                  <button
                    onClick={() => toggleExpand(plan.id, 'edit')}
                    className="px-2 py-1 text-xs rounded bg-black/10 dark:bg-white/10 hover:bg-black/20 dark:hover:bg-white/20"
                  >
                  ...
                  <button
                    ...
                    className="px-2 py-1 text-xs rounded bg-red-500/20 text-red-600 dark:text-red-400 hover:bg-red-500/30 disabled:opacity-50 cursor-not-allowed"
