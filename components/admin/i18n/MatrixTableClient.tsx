    ...
    <section className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`px-3 py-1.5 text-sm rounded-lg border ${filter === 'all' ? 'bg-black/10 dark:bg-white/10 border-black/20 dark:border-white/20' : 'border-black/15 dark:border-white/15'}`}
        >
          {t.admin.languages.filterAll}
        </button>
        <button
          type="button"
          onClick={() => setFilter('empty')}
          className={`px-3 py-1.5 text-sm rounded-lg border ${filter === 'empty' ? 'bg-black/10 dark:bg-white/10 border-black/20 dark:border-white/20' : 'border-black/15 dark:border-white/15'}`}
        >
          {t.admin.languages.filterEmpty}
        </button>
      ...
      <div className="overflow-x-auto rounded-xl border border-black/10 dark:border-white/10">
        <table className="w-full min-w-[960px] text-sm">
          <thead className="bg-black/5 dark:bg-white/5">
            <tr>
              <th className="px-3 py-2 text-left">{t.admin.languages.columnKey}</th>
              <th className="px-3 py-2 text-left">{t.admin.languages.columnEnglish}</th>
              {editableLanguages.map((lang) => (
                <th key={lang.code} className="px-3 py-2 text-left">{lang.name}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {visibleVariables.map((variable) => (
              <tr key={variable.id} className="border-t border-black/10 dark:border-white/10 align-top">
                ...
                <td className="px-3 py-2 bg-black/[0.03] dark:bg-white/[0.04]">
                  <p className="text-xs whitespace-pre-wrap text-foreground/70">{variable.source_text}</p>
                </td>
                {editableLanguages.map((lang) => {
                  const key = cellKey(variable.id, lang.code)
                  const value = draftMap[key] ?? valueMap.get(key)?.value ?? ''
                  ...
                  return (
                    <td key={key} className="px-3 py-2 bg-[#fffef0]">
                      <textarea
                        value={value}
                        onChange={(event) => onValueChange(variable.id, lang.code, event.target.value)}
                        onInput={autoResize}
                        onBlur={() => onBlurSave(variable.id, lang.code)}
                        placeholder={t.admin.languages.notFilled}
                        rows={1}
